...
import type { Eqid, Store } from '@/src/lib/api';
import { storeApi, eqidApi } from '@/src/lib/api';
...
export default function DidStoreDevicesClient({
  ...
  const title = useMemo(() => {
    if (!store) return 'DID · Device';
    return `DID · Device — ${store.agentId || store.agentid}/${store.storeId || store.userid} · ${store.name}`;
  }, [store]);
  ...
  const load = async () => {
    try {
      ...
      if (scopedApi) {
        ...
        s = ctx.store;
        ds = await scopedApi.listEqids();
      } else if (storeRef) {
        s = await storeApi.getById(storeRef);
        ds = await eqidApi.getByStore(storeRef);
      } else {
        throw new Error('storeRef 또는 agentId/storeId가 필요합니다.');
      }
      if (categoryFilter) {
      ...
      <div className="flex items-center justify-between gap-4 mb-4">
        <div className="min-w-0">
          ...
          <p className="text-gray-400 text-sm mt-1">
            좌측은 DID 메뉴, 우측에서 해당 매장의 장치ID(Device ID) 리소스/옵션을 관리합니다.
          </p>
        </div>
