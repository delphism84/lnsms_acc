...
router.post('/login', async (req, res, next) => {
  try {
    // 호환: 기존 agentId/agentid 뿐 아니라, 다른 클라이언트의 userid도 로그인 아이디로 허용
    const agentId = String(req.body.agentId || req.body.agentid || req.body.userid || '').trim();
    const password = req.body.userpw ?? req.body.pw;
    ...
    res.json({
      // 다른 클라이언트 호환(간단 성공 플래그)
      success: true,
      // set-config 클라이언트(lnmsApi.ts) 호환: userid 필드로도 내려줌
      userid: agentId,
      message: '로그인 성공',
      token,
      user: {
