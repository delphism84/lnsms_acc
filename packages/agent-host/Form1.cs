...
public partial class Form1 : Form
{
    ...
    private readonly string _initialUiUrl;
    ...
    public Form1(string initialUiUrl, int agentApiPort)
    {
        _initialUiUrl = initialUiUrl;
        _backendUrl = $"http://localhost:{agentApiPort}";
        _instance = this;
    ...
    private async void NavigateToInitialUi()
    {
        await WaitForHttpAsync(_initialUiUrl);

        if (_webView?.CoreWebView2 != null)
            _webView.CoreWebView2.Navigate(_initialUiUrl);
    }

    private async Task WaitForBackend() => await WaitForHttpAsync(_backendUrl);
    ...
    protected override void OnFormClosing(FormClosingEventArgs e)
    {
        ...
        Services.LocalStackSupervisor.Instance?.Stop();
