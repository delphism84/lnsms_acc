...
static class Program
{
    ...
    private static LocalStackSupervisor? _localStack;
    ...
    static void Main(string[] args)
    {
        ...
        try
        {
            ...
            try
            {
                _localStack = LocalStackSupervisor.StartAsync(cfg).GetAwaiter().GetResult();
            }
            catch (Exception ex)
            {
                MessageBox.Show(
                    $"로컬 LNSMS 스택 기동 실패:\n{ex.Message}\n\nmongod·Node·npm 경로와 app.json repoRoot를 확인하세요.",
                    "LocalStack",
            ...
            Application.ApplicationExit += (_, _) => _localStack?.Stop();

            // 에이전트 API(시리얼·SignalR) 웹서버
            StartWebServer();

            var uiUrl = cfg.BuildStoreSettingUrl();
            ApplicationConfiguration.Initialize();
            Application.Run(new Form1(uiUrl, _port));
        }
        finally
        {
            // 뮤텍스 해제
